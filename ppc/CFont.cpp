...
***********************************************************************************/
//----------------------------------------------------------------------------------
HFONT CFont::CreateFont(HWND hWnd, int height, LPCWSTR faceName, BOOL bold, BOOL italic, BOOL underline, BYTE charset, int angle) {
	HDC hdc = GetDC(hWnd);
	HFONT font = this->CreateFont(hdc, height, faceName, bold, italic, underline, charset, angle);
	ReleaseDC(hWnd, hdc);
	return font;
}
//----------------------------------------------------------------------------------
HFONT CFont::CreateFont(HDC hdc, int height, LPCWSTR faceName, BOOL bold, BOOL italic, BOOL underline, BYTE charset, int angle) {
	if (hdc) {
		height = -height * GetDeviceCaps(hdc, LOGPIXELSY) / 72;
	}else {
	...
	HFONT fnt = CreateFontIndirect(&fn);
