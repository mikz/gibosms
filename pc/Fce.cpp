...
BOOL AddTray(HWND hWnd, UINT ID, UINT callbackMessage, HICON icon, LPTSTR szTip) {
	...
	nd.uCallbackMessage = callbackMessage;
	nd.hIcon = icon;
	_tcscpy(nd.szTip, szTip);
...
BOOL ModifyTray(HWND hWnd, UINT ID, HICON icon) {
	...
	return Shell_NotifyIcon(NIM_MODIFY, &nd);
}
...
char *EncodeURL(char *url) {
	...
	size_t buff_size = strlen(url) + 2*nNonAlNum + 1 + 10000;
	char *encodedUrl = new char[buff_size];
	p = url;
	char *ep = encodedUrl;
	while (*p) {
		if (*p == ' ') {
...
BOOL GetGiboURL(char *url, char *proxy, WCHAR **text, WCHAR **extras, BOOL (* callback)(WPARAM wParam, LPARAM lParam)) {
	...
	DWORD size = 0;
	int err = DownloadToMemory(url, proxy, &data, &size, true, NULL);
	if (err == DTME_NO) {
...
void DrawPictogram(CHdc *hdcPictogram, char *pictogramData, int width, int height) {
	...
	int actBit = 3;
	int actPos = 0;
	int len = (int)strlen(pictogramData);
