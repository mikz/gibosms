...
void XPhonebook::InputAddChar(WCHAR c) {
	int len = this->m_inputItemText ? wcslen(this->m_inputItemText) : 0;
...
void XPhonebook::InputChangeLastChar(WCHAR c) {
	...
	int len = wcslen(this->m_inputItemText);
...
void XPhonebook::InputDrawChar() {
	...
	if (this->m_actInputChar) {
		...
		if (this->InputGetCharData(m_actInputChar, &iStr, &iLetter)) {
			...
			SIZE sz;
			GetTextExtentPoint32(m_hStage, m_keyboardMap[iStr], wcslen(m_keyboardMap[iStr]), &sz);
...
WCHAR XPhonebook::InputGetChar(WCHAR pressedChar, BOOL *replaceActual) {
	if (this->m_input == INPUT::NUMBERS) {
		*replaceActual = false;
		if (pressedChar >= '0' && pressedChar <= '9') return pressedChar;
		else {
			return pressedChar;
		}
	}else if (this->m_input == INPUT::LETTERS) {
		int iStr, iLetter;
...
void XPhonebook::ApplyItemFilter() {
	...
	BOOL t9 = this->m_input == INPUT::NUMBERS;
			...
			do {
				...
				if ((pp = wcschr(seps, *p)) || p == start) {
					...
					if (t9) {
						...
						matches = !wcsnicmp(pp, text, textLen);
					}
...
DWORD XPhonebook::Exists(WCHAR *itemText, BOOL inFiltered) {
	...
	for (DWORD i = 0; i < items->num; i++) {
		if ((*items)[i] && !wcsicmp((*items)[i]->GetText(), itemText)) {
			return i;
		}
	}
	return 0xFFFFFFFF;
