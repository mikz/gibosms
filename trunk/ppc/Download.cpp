...
***********************************************************************************/
int DownloadToMemory(char *lpFrom, char *proxyAddress, char **lplpData, unsigned long *lpSize, int postMethod, int (* callback)(unsigned int wParam, unsigned int lParam)) {
  ...
  *lplpData = NULL;
	...
	if (sock == INVALID_SOCKET) return DTME_CANTCREATESOCKET;
	// call callback procedure and check whether it should continue
	...
	if (proxyAddress && *proxyAddress) {
		...
		if (p && p - proxy > 0) {
			*p = '\0';
			/*int lenEnc = Base64EncodeGetRequiredLength(strlen(proxy), ATL_BASE64_FLAG_NOCRLF);
			authentication = new char[lenEnc + 1];
			Base64Encode((BYTE *)proxy, strlen(proxy), authentication, &lenEnc, ATL_BASE64_FLAG_NOCRLF);
	...
	while (postMethod && bytesSent < requestSize) {
		int bs = send(sock, p, min(chunkSize, requestSize - bytesSent), 0);
		if (bs == SOCKET_ERROR) {
	...
	*lpSize = 0;
	...
	if (data) {
		...
		if (i) {
			memmove(data, data + i, *lpSize - i + 1);
			*lpSize -= i;
		}
	}
	*lplpData = data;
