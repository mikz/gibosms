...
public class SMS extends MIDlet implements CommandListener {
	private final String _VERSION = "2.10";
	...
	private final static Command CMD_SEND = new Command("Poslat", Command.ITEM, 1);
	private final static Command CMD_OK = new Command("OK", Command.ITEM, 1);
	private final static Command CMD_EXIT = new Command("Konec", Command.ITEM, 1);
	...
	protected void startApp() {
		...
		if (setting.srcNum.length() > 0) display.setCurrent(ctrlMessage);
		else showSetting();
	}
  ...
  protected void pauseApp() {
	...
	protected void showAbout() {
		...
		frmAbout.append(new StringItem("Autor:", "Josef Kyrian"));
		...
		if (c == CMD_EXIT) {
			...
			if (isShown(frmSetting)) {
				...
				if (tf.getString().length() < 9) {
					...
					setting.msgsReadPage = Math.max(Global.atoi(((TextField)frmSetting.get(12)).getString()), 1);

					setting.email = ((TextField)frmSetting.get(13)).getString();
